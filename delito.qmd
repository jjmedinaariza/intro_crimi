...
En *Nature and crime*, Marcus @Felson_06 propone un esquema que aspira a resumir las caracteristicas universales de los delitos. @Felson_06 argumenta que todos los delitos tienen una "historia física": un delincuente se mueve hacia su objetivo y entonces actúa sobre este. Para ello se desplaza por una serie de *senderos* para llegar a su objetivo, en ocasiones encontrando *barreras* que tiene que superar a veces usando algunas *herramientas* para finalmente llegar a la *convergencia* con cualquiera que sea su *objetivo*. Estos elementos nos permitirían desarrollar una "taxonomía del delito" en términos de su modus operandi.

```{mermaid}
flowchart LR
  A(Senderos) --> B[Barreras]
  B --> C{Herramientas}
  C --> D(Convergencia)
  D --> E(Objetivo)
```

Los **senderos** pueden ser *físicos*. Un atracador puede usar una autopista, una calle, un camino, para llegar al lugar donde va a cometer su atraco. Pero pueden ser *electrónicos*. Las telecomunicaciones en este sentido han abierto distintas avenidas para la comisión de delitos, pensemos en el ambito digital de la ciberdelincuencia. Finalmente, los senderos pueden ser organizacionales, aludiendo a la forma en que delincuentes pueden aprovecharse de su posición oficial o profesional para acceder a personas, datos, o cosas. Se trata en estos casos de delitos de acceso especializado.

Las **barreras** también pueden ser diversas. @Felson_06 alude a cinco tipos distintos de formas de obstaculizar el "cámino físico" al delito.
